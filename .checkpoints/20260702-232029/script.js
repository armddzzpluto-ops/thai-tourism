...
function showRandomQuote() {
  ...
  if (box && author) {
    ...
    Swal.fire({
      ...
      background: "#111827", color: "#eef2ff", confirmButtonColor: "#00d4aa", confirmButtonText: "Inspiring ✦"
    });
  }
}
...
function toast(msg, icon = "success") {
  if (window.Swal) {
    Swal.fire({
      ...
      background: document.documentElement.getAttribute("data-theme") === "light" ? "#fff" : "#111827",
      color: document.documentElement.getAttribute("data-theme") === "light" ? "#0f172a" : "#eef2ff"
    });
  } else { alert(msg); }
}
...
function initContactForm() {
  ...
  form.addEventListener("submit", e => {
    ...
    if (!name || !email || !message) {
      Swal.fire({ icon: "warning", title: "Missing information", text: "Please fill in your name, email and message.", confirmButtonColor: "#00d4aa", background: "#111827", color: "#eef2ff" });
      return;
    }
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
      Swal.fire({ icon: "error", title: "Invalid email", text: "Please double-check your email address.", confirmButtonColor: "#00d4aa", background: "#111827", color: "#eef2ff" });
      return;
    }
    Swal.fire({
      ...
      confirmButtonColor: "#00d4aa", confirmButtonText: "Great, thanks!",
      background: "#111827", color: "#eef2ff"
    }).then(() => form.reset());
  });
}
